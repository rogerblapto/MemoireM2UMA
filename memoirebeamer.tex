...
\begin{frame}{Esquisse de la preuve : irréductibilité de \texorpdfstring{$\varphi_I$}{phi\_I} (2/2)}
	\begin{enumerate} [label=\roman*), start=3]
		\item[] \textbf{\underline{Montrons que le produit tensoriel des représentations linéaires non triviales}\\
		\item[] \textbf{ \( \varphi_i \) est irréductible} :}\\
		Soit \( J \subset I \) l'ensemble des indices correspon
		...
		\]
		Puisque \( \bigotimes_{i \notin J} V_i \) est constitué de représentations triviales, il n’affecte pas l'irréductibilité. Par conséquent, l'irréductibilité de \( \varphi \) entraîne celle de \( \bigotimes_{i \in J} \varphi_i \).\\
		
		\textbf{\underline{Sens réciproque}}\\
		\textbf{Montrons que \( \varphi \) est irréductible :}\\
...
Par hypothèse, \( \bigotimes_{i \in J} \varphi_i \) est irréductible. Donc \( W = \{0\} \) ou \( \bigotimes_{i \in I} V_i \).\\
